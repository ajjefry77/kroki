  ...
  <div class="landing min-h-screen flex flex-col overflow-x-clip">
    ...
    >
      ...
      >
        ...
        >
          <div class="flex items-center gap-3 justify-self-start min-w-0">
            <span class="logo-3d relative shrink-0">
              <img src="/favicon.png" alt="لوگوی سامانه کروکی" class="w-10 h-10 rounded-xl object-contain shadow-lg shadow-[var(--accent-glow-strong)] transition-transform duration-500" :class="scrolled ? 'scale-90' : 'scale-100'" />
              <span class="logo-ring"></span>
            </span>
            <div class="min-w-0 transition-all duration-500 overflow-hidden" :class="scrolled ? 'hidden sm:block' : 'block'">
              <div class="font-extrabold text-lg leading-tight whitespace-nowrap">سامانه کروکی</div>
          ...
          <div class="flex items-center gap-2 justify-self-end">
            <button v-if="!authed" class="btn btn-ghost h-9 !rounded-full transition-all duration-500" :class="scrolled ? '!px-4 !text-[13px]' : ''" @click="$emit('login')">
              <i class="fas fa-right-to-bracket ml-1"></i>
              <span class="hidden sm:inline">ورود / ثبت‌نام</span><span class="sm:hidden">ورود</span>
            </button>
        ...
        <div class="absolute bottom-0 right-0 left-0 h-[2.5px] bg-transparent overflow-hidden" :class="scrolled ? 'rounded-b-[999px]' : ''">
          <div class="h-full bg-gradient-to-l from-[var(--accent-soft)] to-[var(--accent)] transition-[width] duration-150 ease-out" :style="{ width: scrollProgress + '%' }"></div>
        </div>
      </div>
    </header>
    <!-- فاصله‌گذار برای هدر فیکس تا محتوا زیر آن نرود -->
    <div class="shrink-0 transition-all duration-500" :class="scrolled ? 'h-[72px]' : 'h-16'"></div>
    ...
    <!-- هیرو سه‌بعدی -->
    <section ref="heroSection" class="relative flex-1 flex items-center justify-center py-16 md:py-24 overflow-hidden" @mousemove="onHeroMove" @mouseleave="resetHero">
      ...
      <div ref="orbs" class="absolute inset-0 pointer-events-none">
        ...
        <div data-depth="70" class="float-chip chip-1"><i class="fas fa-location-dot"></i></div>
        <div data-depth="90" class="float-chip chip-2"><i class="fas fa-ruler-combined"></i></div>
        <div data-depth="45" class="float-chip chip-3"><i class="fas fa-compass-drafting"></i></div>
      ...
      <div class="relative max-w-7xl mx-auto px-5 text-center w-full">
        <div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-[var(--accent)]/40 bg-[var(--accent-glow)] text-[var(--accent-soft)] text-xs font-medium mb-6 reveal badge-3d">
          <i class="fas fa-certificate"></i>
          رسمی، سازمانی و با استانداردهای فنی
        </div>

        <h1 class="text-4xl md:text-6xl font-black leading-tight mb-5 reveal hero-title">
          کروکی نقشه ملک خود را
          <span class="title-3d text-transparent bg-clip-text bg-gradient-to-l from-[var(--accent-soft)] to-[var(--accent)]">حرفه‌ای</span>
          بسازید
        </h1>
        ...
        <div class="flex flex-col sm:flex-row items-center justify-center gap-3 mb-12 reveal">
          ...
          <a href="#how" class="btn btn-ghost !px-8 !py-3 !text-base !rounded-xl !bg-white/60 !backdrop-blur">
            مشاهده مراحل
        ...
        <div class="scene relative mx-auto max-w-3xl reveal" @mousemove="onMockupMove" @mouseleave="resetMockup">
          <div class="absolute -inset-6 bg-[var(--accent-glow)] blur-3xl rounded-full pointer-events-none"></div>
          <div ref="mockupInner" class="tilt-card relative bg-[var(--surface)] border border-[var(--border-strong)] rounded-2xl shadow-2xl overflow-hidden will-change-transform">
            <div class="flex items-center gap-2 px-4 py-2.5 bg-[var(--surface2)] border-b border-[var(--border)]">
              ...
              <span class="mx-auto text-[11px] text-[var(--text-muted)]">پیش‌نمایش کروکی — <span class="text-[var(--accent)] font-bold">نمای سه‌بعدی تعاملی</span></span>
            </div>
        ...
        <p class="mt-4 text-[11px] text-[var(--text-faint)] reveal">نشانگر موس را روی پیش‌نمایش حرکت دهید — کارت به‌صورت سه‌بعدی می‌چرخد</p>
      </div>
    </section>

    <!-- امکانات سه‌بعدی -->
    <section id="features" class="py-16 md:py-20 border-t border-[var(--border)] bg-[var(--bg-elevated)]/50 relative overflow-hidden">
      ...
      <div class="relative max-w-7xl mx-auto px-5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5" style="perspective: 1200px">
          <div
            v-for="f in features"
            :key="f.title"
          ...
          >
            ...
            <div class="card-inner">
              <div class="icon-3d w-12 h-12 rounded-xl bg-[var(--accent-glow)] border border-[var(--accent)]/30 flex items-center justify-center mb-4">
    ...
    <!-- قالب‌ها -->
    <section id="templates" class="py-16 md:py-20 border-t border-[var(--border)] relative overflow-hidden">
      <div class="relative max-w-7xl mx-auto px-5">
        <div class="text-center mb-12 reveal">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3" style="perspective: 1000px">
          <div
            ...
            @mouseleave="resetCardTilt"
          >
            <div class="w-10 h-10 mx-auto rounded-lg mb-3 flex items-center justify-center pop-3d" style="background: rgba(250, 108, 4, 0.14); color: var(--accent)">
          ...
          <div
            ...
            @mouseleave="resetCardTilt"
          >
            <div class="w-10 h-10 mx-auto rounded-lg mb-3 flex items-center justify-center pop-3d" :style="{ background: 'rgba(' + hexToRgb(t.headerColor) + ',0.14)', color: t.headerColor }">
    ...
    <!-- مراحل -->
    <section id="how" class="py-16 md:py-20 border-t border-[var(--border)] bg-[var(--bg-elevated)]/50 relative overflow-hidden">
      <div class="relative max-w-7xl mx-auto px-5">
        <div class="text-center mb-12 reveal">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4" style="perspective: 1000px">
          <div v-for="(s, i) in steps" :key="s.title" class="relative reveal step-3d" @mousemove="onCardTilt" @mouseleave="resetCardTilt">
            <div class="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 h-full relative overflow-hidden">
              <div class="step-glow"></div>
              <div class="num-3d w-9 h-9 rounded-full bg-gradient-to-br from-[var(--accent)] to-[var(--accent-soft)] text-[#241a05] font-extrabold flex items-center justify-center mb-3 shadow-md shadow-[var(--accent-glow)]">
                {{ i + 1 }}
              </div>
...
.section-glow {
  background: radial-gradient(ellipse 60% 50% at 50% 0%, rgba(250,108,4,0.06), transparent 70%);
}
...
.badge-3d {
  box-shadow: 0 6px 20px rgba(250,108,4,0.15), inset 0 1px 0 rgba(255,255,255,0.6);
  transform: translateZ(30px);
}
/* دکمه‌های اصلی ثابت هستند (بدون حرکت مگنتیک/شناور) */
.btn-static {
  box-shadow: 0 10px 28px rgba(250,108,4,0.28), inset 0 1px 0 rgba(255,255,255,0.35);
}
...
.orb-2 {
  ...
  bottom: 8%; left: 4%;
  background: radial-gradient(circle, rgba(47,111,208,0.22), transparent 70%);
...
.chip-1 { top: 14%; right: 12%; animation: floatY 5s ease-in-out infinite; }
.chip-2 { top: 46%; left: 7%; animation: floatY 6s ease-in-out infinite 0.6s; color: var(--info); }
.chip-3 { bottom: 18%; right: 8%; animation: floatY 5.5s ease-in-out infinite 1.1s; color: #1d3a6e; }
.float-ring {
  position: absolute;
  border-radius: 999px;
...
.ring-1 { width: 120px; height: 120px; top: 20%; left: 14%; animation: spinSlow 22s linear infinite; }
.ring-2 { width: 70px; height: 70px; bottom: 26%; left: 20%; animation: spinSlow 16s linear infinite reverse; border-color: rgba(47,111,208,0.3); }
...
.badge-top { top: -18px; right: 8%; animation: floatY 4.5s ease-in-out infinite; }
.badge-right { bottom: 32%; right: -24px; animation: floatY 5.2s ease-in-out infinite 0.5s; }
.badge-left { bottom: 12%; left: -14px; animation: floatY 4.8s ease-in-out infinite 1s; }
.dot { width: 8px; height: 8px; border-radius: 999px; display: inline-block; }
.dot-green { background: #1fa15c; box-shadow: 0 0 0 4px rgba(31,161,92,0.15); animation: pulse 2s infinite; }
.dot-orange { background: var(--accent); box-shadow: 0 0 0 4px rgba(250,108,4,0.15); animation: pulse 2s infinite; }
...
.feature-card:hover {
  ...
  box-shadow: 0 18px 48px rgba(23, 43, 77, 0.12), 0 4px 14px rgba(250,108,4,0.1);
}
.card-shine {
  position: absolute;
  inset: 0;
  background: radial-gradient(420px circle at var(--mx, 50%) var(--my, 50%), rgba(250,108,4,0.1), transparent 60%);
  opacity: 0;
  transition: opacity 0.25s ease;
  pointer-events: none;
}
.feature-card:hover .card-shine { opacity: 1; }
.card-inner { transform: translateZ(24px); transform-style: preserve-3d; }
.icon-3d {
  transform: translateZ(36px);
  box-shadow: 0 8px 20px rgba(250,108,4,0.18), inset 0 1px 0 rgba(255,255,255,0.7);
  transition: transform 0.25s var(--ease-out);
}
.feature-card:hover .icon-3d { transform: translateZ(48px) rotate(-6deg) scale(1.05); }
.pop-3d { transform: translateZ(30px); box-shadow: 0 8px 18px rgba(23,43,77,0.1); }
.template-3d {
  transform-style: preserve-3d;
  transition: transform 0.18s ease-out, box-shadow 0.25s, border-color 0.25s !important;
  will-change: transform;
...
.step-3d { transform-style: preserve-3d; }
.step-3d > div {
  transition: transform 0.18s ease-out, box-shadow 0.25s, border-color 0.25s;
  will-change: transform;
}
.step-glow {
  position: absolute;
  inset: auto -20% -40% -20%;
  height: 120px;
  background: radial-gradient(ellipse at center, rgba(250,108,4,0.12), transparent 70%);
  pointer-events: none;
}
.num-3d { transform: translateZ(28px); }
...
.cta-ring-2 { animation-delay: 2s; width: 620px; height: 620px; }
...
@media (prefers-reduced-motion: reduce) {
  .orb, .float-chip, .float-badge, .cta-ring, .float-ring {
    animation: none !important;
  }
  .tilt-card, .feature-card, .template-3d {
